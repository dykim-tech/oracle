...
비밀번호는 GitHub에 저장되지 않으며 DBCA 단계에서 화면에 표시하지 않고 입력받습니다. 자동화가 필요하면 실행 환경에서 `SYS_PASSWORD`, `SYSTEM_PASSWORD`, `PDB_ADMIN_PASSWORD`를 설정하고 `sudo -E ./install.sh`를 사용하세요. 셸 히스토리에 비밀번호를 직접 입력하지 않는 것을 권장합니다.
